...
- 일별 actual 완료 판정은 스냅샷 생성 시각이 아니라 최신 일별 `priceDate` 기준으로 처리합니다.
...
```
대상:
- WTI: FRED `DCOILWTICO` 일별
- Brent: FRED `DCOILBRENTEU` 일별
- Dubai: 한국석유공사 오피넷 `Dubai 현물가격 추정값` 일별
- USD/KRW: FRED `DEXKOUS` 일별

동작 규칙:
- 공개 시장 요인인 두바이유와 USD/KRW는 각 원천의 최신 유효 일별 관측값 두 건만 사용합니다.
- 두바이유는 오피넷 국제유가 원유 화면의 USD/BBL 일별 Dubai 값을 사용하며, 월간 FRED `POILDUBUSDM`은 공개 카드 계산에서 제외합니다.
- USD/KRW는 FRED `DEXKOUS` 뉴욕 정오 매입환율에서 빈 값, `.`, 비정상 값, 0 이하 값을 제외합니다.
- 주말·공휴일에는 값을 만들지 않고 원천에 존재하는 가장 최근 두 거래일을 사용합니다.
- 관측 기준일은 원천의 실제 날짜이며 실행일과 수집 시각은 `collectedAt` 및 동기화 상태로만 관리합니다.
- 각 수집 실행은 두 지표를 독립적으로 갱신합니다. 실패한 지표는 마지막 정상 관측값을 유지하고 대시보드에 `최신 데이터 확인 중`을 표시합니다.
...
운영 점검 체크리스트:
- Vercel `DATABASE_URL`과 GitHub Actions `DATABASE_URL`은 같은 production DB를 가리켜야 합니다.
- GitHub Actions에서 `scheduled-opinet-worker` workflow가 `disabled` 상태가 아닌지 확인합니다.
- `workflow_dispatch`로 수동 1회 실행해 외부 지표 sync와 오피넷 ingest가 모두 성공하는지 먼저 검증합니다.
- 실행 후 production 대시보드의 `데이터 출처 및 유의사항` 영역에서 오피넷 경유가, 두바이유, USD/KRW의 실제 일별 관측 기준일이 갱신됐는지 확인합니다.
- 두바이유와 USD/KRW의 관측일은 각 시장의 최근 유효 거래일이며 수집 실행일과 다를 수 있습니다.

수동 확인 순서:
1. GitHub `Actions` → `scheduled-opinet-worker` → `Run workflow`
2. 실행 로그에서 `npm run sync:indicators`와 `npm run ingest:opinet`가 모두 성공했는지 확인
3. `/api/fsc/current`와 홈 화면에서 최신 데이터 기준 시각, 오피넷 값, 출처 관측 기준 갱신 여부 확인
