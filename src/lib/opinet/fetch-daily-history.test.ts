...
test('mergeOpinetDieselDailyHistory prefers recent rows for overlapping dates', () => {
  const merged = mergeOpinetDieselDailyHistory(currentRows, recentRows);

  assert.equal(merged.length, 2);
  assert.deepEqual(
    merged.map((row) => ({ date: row.date, price: row.price, diff: row.diff })),
    [
      { date: '20260712', price: 1865.99, diff: -1.22 },
      { date: '20260713', price: 1863.93, diff: -2.06 },
    ],
  );
});
