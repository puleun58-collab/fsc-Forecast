...
test('dashboard data sources include only public sources and keep dynamic observation times', () => {
  const dataSources = buildDashboardDataSources({
    latestOpinetObservedAt: '2026-07-14',
  ...
  assert.equal(dataSources[0].latestObservedAt, '2026-07-14');
  assert.equal(dataSources[0].latestCollectedAt, '2026-07-14T01:25:00.000Z');
  assert.equal(dataSources[1].latestObservedAt, '2026-06-01T00:00:00.000Z');
  assert.equal(dataSources[1].latestCollectedAt, '2026-07-10T00:00:00.000Z');
  assert.equal(dataSources[2].latestObservedAt, '2026-07-10T00:00:00.000Z');
  assert.equal(dataSources[2].latestCollectedAt, '2026-07-15T00:00:00.000Z');
