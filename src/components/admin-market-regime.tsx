    ...
    >
      <div className="admin-detail-stack">
        ...
        <div className="admin-table-wrap">
          <table className="admin-table market-regime-table">
            <caption className="admin-table__caption">
              최근 {analysis.windowWeeks}주 1주 예측 기준 · 전체 MAE{' '}
        ...
        {analysis.regimes.some(
          (summary) =>
            summary.sampleCount > 0 && summary.sampleCount < MARKET_REGIME_MIN_REPORT_SAMPLE_COUNT,
        ) ? (
        ...
        <div className="admin-panel">
          ...
          {weakest === null ? (
            ...
            <p className="admin-decision__note">
              ...
              {formatMae(weakest.maeKrwPerL)} · 최대 오차 {formatMae(weakest.maxAbsoluteErrorKrwPerL)}
              {weakestGap === null ? '' : ` · 전체 대비 ${weakestGap > 0 ? '+' : ''}${formatPriceText(weakestGap)}`}
            </p>
          )}
        </div>
                  ...
                  <tbody>
                    {analysis.largestErrors.map((entry) => (
                      <tr key={entry.targetDate}>
                        ...
                        <td data-label="Forecast">{formatPriceText(entry.forecastKrwPerL)}</td>
        ...
        <p className="admin-decision__note">
          국면별 성능은 Forecast 특성을 이해하기 위한 참고 진단이며 공식 신뢰도 등급과 별도로
          표시됩니다.
        </p>
      </div>
    </SectionCard>
