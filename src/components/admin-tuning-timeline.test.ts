...
test('each event states its week, title and candidate', () => {
  ...
  assert.match(markup, /Cap ±1%/);
  assert.match(markup, /1\/2주 확인/);
});

test('the newest event is the only one marked as current', () => {
  const markup = render([
  ...
  assert.equal(markup.match(/status-tag--accent">현재/g)?.length, 1);
...
test('a candidate change shows the setting it replaced', () => {
  ...
  assert.match(markup, /1순위 후보 변경/);
  assert.match(markup, /이전 후보 · .*Cap ±1%/);
  assert.match(markup, /Trend 6주/);
});
