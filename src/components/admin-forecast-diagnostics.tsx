      ...
      <div className="admin-detail-stack">
        <div className="admin-metric-grid">
          {[
            ...
            [
              "최근 승격일",
              diagnostics.promotedAt === null
                ? "승격 이력 없음"
                : formatDateTime(diagnostics.promotedAt),
            ],
        ...
        <div className="admin-panel">
          ...
          <span>
            외부 보정 상한: ±
            {(selectedParams.externalAdjustmentCapRatio * 100).toFixed(1)}%
          </span>
        </div>
          ...
          {reliability === null && diagnostics.recentOneStep === null ? (
            ...
            <div className="admin-decision__metrics admin-reliability__metrics">
              <DiagnosticMetric
                label="MAPE"
                value={formatPercent(
                  reliability?.recent13wWeeklyPriceMape ??
                    diagnostics.recentOneStep?.mapePct ??
                    null,
                )}
              />
              <DiagnosticMetric
                label="MAE"
                value={formatMetric(
                  reliability?.recent13wWeeklyPriceMae ??
                    diagnostics.recentOneStep?.maeKrwPerL ??
                    null,
                  2,
                  "원/L",
                )}
        ...
        <div className="admin-panel admin-decision">
          <div className="admin-decision__header">
            ...
            <details className="admin-thresholds">
              <summary className="status-tag status-tag--interactive">
                승격 기준 보기
              </summary>
              <div className="admin-thresholds__panel">
                <p>
                  모델 승격은 최근 예측 성능 개선뿐 아니라 장기 안정성, 최대
                  오차, 예측 변동성, 승격 쿨다운을 함께 고려합니다.
                </p>
        ...
        <details className="admin-panel admin-disclosure">
          ...
          <div className="admin-disclosure__body">
            <div className="admin-decision__metrics admin-decision__metrics--wide admin-disclosure__metrics">
              <DiagnosticMetric
                label="전체 예측 MAPE"
                value={formatPercent(diagnostics.recent?.mapePct ?? null)}
              />
              <DiagnosticMetric
                label="전체 예측 MAE"
                value={formatMetric(
                  diagnostics.recent?.maeKrwPerL ?? null,
                  2,
                  "원/L",
                )}
              />
              <DiagnosticMetric
                label="최대 오차"
                value={formatMetric(
                  diagnostics.recent?.maxAbsoluteErrorKrwPerL ?? null,
                  2,
                  "원/L",
                )}
              />
              <DiagnosticMetric
                label="예측 변동성"
                value={formatMetric(
                  diagnostics.recent?.forecastChurnKrwPerL ?? null,
                  2,
                  "원/L",
                )}
              />
              <DiagnosticMetric
                label="장기 예측 MAE"
                value={formatMetric(
                  diagnostics.long?.maeKrwPerL ?? null,
                  2,
                  "원/L",
                )}
              />
              <DiagnosticMetric
                label="평가 개수"
                value={describeSampleCount(
                  diagnostics.recent?.sampleCount ?? 0,
                )}
              />
            </div>
            <p className="admin-decision__note">
              ...
              <div className="admin-table-wrap">
                <table className="admin-table diagnostics-metrics-table">
                  <thead>
                    <tr>
                      <th scope="col">모델</th>
                      <th scope="col">전체 예측 MAE</th>
                      <th scope="col">전체 예측 MAPE</th>
                      <th scope="col">장기 예측 MAE</th>
                      <th scope="col">최대오차</th>
                      <th scope="col">평가 개수</th>
                    </tr>
                  </thead>
                  <tbody>
                    {diagnostics.candidates.map((candidate) => (
                      <tr key={`all-horizon-${candidate.modelId}`}>
                        <th scope="row" data-label="모델">
                          Model {candidate.modelId}
                        </th>
                        <td data-label="전체 예측 MAE">
                          {formatMetric(candidate.recentMaeKrwPerL, 2, "원/L")}
                        </td>
                        <td data-label="전체 예측 MAPE">
                          {formatPercent(candidate.recentMapePct)}
                        </td>
                        <td data-label="장기 예측 MAE">
                          {formatMetric(candidate.longMaeKrwPerL, 2, "원/L")}
                        </td>
                        <td data-label="최대오차">
                          {formatMetric(
                            candidate.maxAbsoluteErrorKrwPerL,
                            2,
                            "원/L",
                          )}
                        </td>
                        <td data-label="평가 개수">
                          {describeSampleCount(candidate.recentSampleCount)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="admin-table-wrap">
              <table className="admin-table diagnostics-metrics-table">
                <thead>
                  <tr>
                    <th scope="col">모델</th>
                    <th scope="col">최근 13주 MAE</th>
                    <th scope="col">최근 13주 MAPE</th>
                    <th scope="col">최근 26주 MAE</th>
                    <th scope="col">13주 최대오차</th>
                    <th scope="col">표본</th>
                    <th scope="col">상태</th>
                  </tr>
                </thead>
                <tbody>
                  {diagnostics.candidates.map((candidate) => (
                    <tr key={candidate.modelId}>
                      ...
                      <td data-label="최근 13주 MAE">
                        {formatMetric(
                          candidate.recentOneStepMaeKrwPerL,
                          2,
                          "원/L",
                        )}
                      </td>
                      <td data-label="최근 13주 MAPE">
                        {formatPercent(candidate.recentOneStepMapePct)}
                      </td>
                      <td data-label="최근 26주 MAE">
                        {formatMetric(
                          candidate.longOneStepMaeKrwPerL,
                          2,
                          "원/L",
                        )}
                      </td>
                      <td data-label="13주 최대오차">
                        {formatMetric(
                          candidate.recentOneStepMaxAbsoluteErrorKrwPerL,
                          2,
                          "원/L",
                        )}
                      </td>
                      <td data-label="표본">
                        {candidate.recentOneStepSampleCount === 0
                          ? MISSING_METRIC_TEXT
                          : `${candidate.recentOneStepSampleCount}주`}
                      </td>
                      <td data-label="상태">
                        <span
              ...
              {diagnostics.candidates.map((candidate) => (
                <li key={`params-${candidate.modelId}`} className="admin-panel">
                  ...
                  <span>
                    외부 보정 상한: ±
                    {(
                      candidate.params.externalAdjustmentCapRatio * 100
                    ).toFixed(1)}
                    %
                  </span>
                </li>
          ...
          {modelHistoryGroups.length === 0 ? (
            ...
            <>
              <ul className="model-history-list">
                {modelHistoryGroups
                  .slice(0, MODEL_HISTORY_PREVIEW_COUNT)
                  .map((group) => (
                    <ModelHistoryRow
                      key={group.key}
                      ...
                      resultText={
                        group.count > 1
                          ? `${group.resultText} · ${group.count}회`
                          : group.resultText
                      }
