...
- **FscQuarterWeekDto** (`FscResultDto.weeks` 및 주차 route의 각 `weeks` 원소): `sequenceNo: number`, `targetMonth: number`, `weekNo: number`, `weekStartDate: ISO string`, `weekEndDate: ISO string`, `priceKind: string`, `priceKrwPerL: string(2)|null`, `actualPriceKrwPerL: string(2)|null`, `forecastPriceKrwPerL: string(2)|null`, `forecastLowerBoundKrwPerL: string(2)|null`, `forecastUpperBoundKrwPerL: string(2)|null`, `sourcePriceDate: ISO string|null`, `forecastSourceKind: "weekly_point"|"weekly_trend_extension"|null`, `fallbackUsed: boolean`, `basePriceKrwPerL: string(2)`, `priceDiffKrwPerL: string(2)|null`, `diffRatio: string(6)|null`. 주간 예측 이력이 부족해 추세를 계산할 수 없는 Forecast 주차는 가격·차이·source가 `null`이며 화면에서는 `예측값 산정 중`으로 표시한다. `forecastLowerBoundKrwPerL`·`forecastUpperBoundKrwPerL`는 해당 horizon의 walk-forward 90% 절대오차 분위수로 산정한 예상 범위이며, 백테스트 표본이 없으면 `null`이다.
