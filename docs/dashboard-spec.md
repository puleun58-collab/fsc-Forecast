...
6. **WeeklyDetailTable / WeeklyOutlookTable:** 현재 분기에서는 주차별 가격·기간·상태·전주 대비 변화·기준 대비 차이·차이율·산출 방식을 표시한다. 향후 전망 표에서는 최신 Actual 1주와 Forecast 13주의 전주 대비 변화, 기준유가 대비 차이, 단기·중기·장기 구간과 신뢰 범위를 표시한다.
...
향후 13주 전망은 활성 분기의 FSC 주차를 연장하지 않는다. 최신 완료 actual 주차를 기준으로 최근 actual 최대 4주와 forecast run의 weekly point 최대 13주를 별도 rolling view로 구성한다. 차트는 추세 맥락을 위해 recent actual 최대 4주를 유지하고, 표는 최신 Actual 1주와 Forecast 13주만 표시한다. 주간 전망은 최신 actual을 기준점으로 최근 평균 변화량을 적용하되 장기 구간으로 갈수록 변화폭을 감쇠한다. 1~4주는 단기, 5~8주는 중기, 9~13주는 장기 전망으로 구분하며 forecast point에 하단·상단 범위가 있을 때만 차트 음영과 범위 값을 표시한다.
...
상세 표는 actual과 forecast의 첫 경계 앞에 `예측 시작` 행을 넣는다. 각 행은 순번, ISO 주 번호와 월, 기간, 상태, 가격, 전주 대비 변화, 기준유가 대비 차이·차이율, 산출 방식을 표시한다. forecast 산출 방식은 주간 예측값, 월간 예측값, 직전 예측값 유지, 현재 적용유가 대체, 기준유가 대체로 구분한다. 데스크톱 표는 fallback 행에 `대체값 사용`과 source kind를 표시하지만, 현재 모바일 그룹에는 이 provenance가 표시되지 않는다. 모바일에서는 Actual 구간과 Forecast 구간만 별도 그룹으로 표시한다.
