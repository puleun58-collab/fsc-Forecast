...
5. **WeeklyAnalysis:** `현재 분기`와 `13주 전망`을 전환한다. 현재 분기에서는 FSC 산출에 사용된 actual/forecast 추이와 기준유가 선을 표시하고, 13주 전망에서는 최신 완료 actual 1주와 weekly forecast 13주, 예측 평균·범위·방향을 표시한다. 상단 그래프 제목은 `13주 유가 전망`, 하단 상세 표 제목은 `주차별 전망`으로 구분한다.
6. **WeeklyDetailTable / WeeklyOutlookTable:** 현재 분기에서는 주차별 가격·기간·상태·전주 대비 변화·기준 대비 차이·차이율·산출 방식을 표시한다. 분기 첫 행의 전주 대비는 직전 완료 오피넷 주간값을 이어서 계산한다. 향후 전망 표에서는 최신 Actual 1주와 Forecast 13주의 전주 대비 증감액·증감률·방향, 기준유가 대비 차이, 단기·중기·장기 구간과 신뢰 범위를 표시한다.
...
향후 13주 전망은 활성 분기의 FSC 주차를 연장하지 않는다. 최신 완료 actual 1주와 forecast run의 weekly point 최대 13주를 차트와 표에 동일하게 표시한다. 주간 전망은 최신 actual을 기준점으로 최근 평균 변화량을 적용하되 장기 구간으로 갈수록 변화폭을 감쇠한다. 1~4주는 단기, 5~8주는 중기, 9~13주는 장기 전망으로 구분하며 forecast point에 하단·상단 범위가 있을 때만 차트 음영과 범위 값을 표시한다.

현재 분기의 FSC 계산은 기존 규칙을 유지하기 위해 forecast horizon 1~4까지만 주간 예측값을 직접 사용한다. 이후 주차는 월간 예측값을 우선 사용하고, 값이 없을 때 직전 예측값 유지와 설정 유가 fallback 순서로 전환한다. 따라서 13주 rolling 전망을 추가해도 현재 분기 전체가 주간 선형 추세로 대체되지 않는다.

상세 표는 actual과 forecast의 첫 경계 앞에 `예측 시작` 행을 넣는다. 각 행은 순번, ISO 주 번호와 월, 기간, 상태, 가격, 전주 대비 증감액·증감률·방향, 기준유가 대비 차이·차이율, 산출 방식을 표시한다. 분기 첫 주는 계산 payload에 저장한 직전 완료 주간 공식값을 비교 기준으로 사용하고, 공식값이 없으면 완성된 5일 일별 평균을 사용한다. forecast 산출 방식은 주간 예측값, 월간 예측값, 직전 예측값 유지, 현재 적용유가 대체, 기준유가 대체로 구분한다. 데스크톱 표는 fallback 행에 `대체값 사용`과 source kind를 표시하지만, 현재 모바일 그룹에는 이 provenance가 표시되지 않는다. 모바일에서는 Actual 구간과 Forecast 구간만 별도 그룹으로 표시한다.
...
주요 시장 요인은 Dubai와 USD/KRW의 공개 일별 유효 관측값이다. USD/KRW는 ECB가 영업일마다 고시하는 EUR 기준 USD·KRW 환율의 교차환율(KRW/EUR ÷ USD/EUR)을 사용한다. 변화·비율·방향을 계산하려면 각 지표에 서로 다른 관측일의 최신값과 직전값이 모두 필요하며, 두 지표가 이 조건을 충족해야 시장 요인 상태가 준비됨이다. 하나라도 부족하면 `insufficient_data`, 확인 중인 지표가 있으면 `checking`으로 표시한다. 각 카드에는 값, 전일 대비 변화·비율·방향, 관측일, provider와 값 기준을 표시하며 FSC 산출 근거로 표현하지 않는다.
