...
각 시장 요인 카드는 최신 관측일 기준 최근 30일의 실제 일별 관측값으로 미니 추세 차트를 함께 표시한다. 결측일은 보간하지 않고 저장된 관측값만 연결하며, 시작일·최신일·최소·최대값·최신값 dot만 표시하고 범례와 축 제목, 확대 컨트롤은 두지 않는다. hover 또는 tap하면 `관측일`과 `지표명 값 단위` tooltip을 표시한다. 유효 관측값이 3개 미만이면 `추세 데이터 준비 중`으로 표시하고, 창 전체를 덮지 못하면 `최근 데이터 {n}일`로 실제 표본 수를 밝힌다. 데스크톱·태블릿에서는 카드 왼쪽에 지표 정보, 오른쪽에 차트를 두고 카드 폭이 좁아지면 지표 정보 아래로 차트를 세로 배치한다.
